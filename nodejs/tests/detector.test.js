...
import { describe, it, expect, beforeAll, afterAll } from '@jest/globals'; // optional — jest globals are auto-injected
...
describe('exportCsv', () => {
  it('writes a CSV with correct headers and data', async () => {
    ...
    const results = [
      { path: join(tmpDir, 'a.txt'), encoding: 'UTF-8',      confidence: 0.99, sizeBytes: 100, error: null },
      { path: join(tmpDir, 'b.txt'), encoding: 'ISO-8859-1', confidence: 0.87, sizeBytes: 200, error: null },
      { path: join(tmpDir, 'c.bin'), encoding: null,          confidence: null, sizeBytes: 0,   error: 'Permission denied' },
    ];
